...
const saveLimiter = submissionLimiter(60, 'Đồng bộ Passport quá nhiều. Vui lòng thử lại sau 15 phút.')
...
router.put('/:key', saveLimiter, async (req, res) => {
    if (!KEY_PATTERN.test(req.params.key)) return res.status(400).json({ error: 'Passport key không hợp lệ' })
    if (!req.body || typeof req.body !== 'object' || Array.isArray(req.body)
        || JSON.stringify(req.body).length > 250_000) {
        return res.status(400).json({ error: 'Dữ liệu Passport không hợp lệ hoặc quá lớn' })
    }
    try {
